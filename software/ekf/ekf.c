...
void ekf_predict(ekf_t *ekf, ekf_control_t *c){
    ...
    gsgt[0][1] = gsgt[1][0] = ekf->cov[0][1] + ekf->G[0][2]*ekf->cov[2][1] + ekf->G[1][2]*ekf->cov[0][2] + ekf->G[0][2]*ekf->G[1][2]*ekf->cov[2][2];
...
void ekf_correct(ekf_t *ekf, ekf_measurement_t *m){
    ...
    sigma_ht[0][1] = ekf->cov[0][0]*ekf->H[1][0] + ekf->cov[0][1]*ekf->H[1][1] + ekf->cov[0][2]*ekf->H[2][2];
    sigma_ht[1][0] = ekf->cov[1][0]*ekf->H[0][0] + ekf->cov[1][1]*ekf->H[0][1] + ekf->cov[1][2]*ekf->H[0][2];
    sigma_ht[1][1] = ekf->cov[1][0]*ekf->H[1][0] + ekf->cov[1][1]*ekf->H[1][1] + ekf->cov[1][2]*ekf->H[2][2];
    sigma_ht[2][0] = ekf->cov[2][0]*ekf->H[0][0] + ekf->cov[2][1]*ekf->H[0][1] + ekf->cov[2][2]*ekf->H[0][2];
    sigma_ht[2][1] = ekf->cov[2][0]*ekf->H[1][0] + ekf->cov[2][1]*ekf->H[1][1] + ekf->cov[2][2]*ekf->H[2][2];
    ...
    float K[3][2];
    K[0][0] = h_sigma_ht[0][0]*D_inv[0][0] + h_sigma_ht[0][1]*D_inv[1][0];
    K[0][1] = h_sigma_ht[0][0]*D_inv[0][1] + h_sigma_ht[0][1]*D_inv[1][1];
    K[1][0] = h_sigma_ht[1][0]*D_inv[0][0] + h_sigma_ht[1][1]*D_inv[1][0];
    K[1][1] = h_sigma_ht[1][0]*D_inv[0][1] + h_sigma_ht[1][1]*D_inv[1][1];
    K[2][0] = h_sigma_ht[2][0]*D_inv[0][0] + h_sigma_ht[2][1]*D_inv[1][0];
    K[2][1] = h_sigma_ht[2][0]*D_inv[0][1] + h_sigma_ht[2][1]*D_inv[1][1];
...
static void ekf_dg_dcontrol(ekf_t *ekf, ekf_control_t *c){
    ...
    if (c->r != c->l){
        ...
        ekf->V[0][0] = w*c->r/rml2*(sin(theta_)-sin(ekf->theta)) - rpl/(2*rml)*cos(theta_);
        ekf->V[0][1] = w*c->r/rml2*(-cos(theta_)+cos(ekf->theta)) - rpl/(2*rml)*sin(theta_);
        ekf->V[0][2] = -1.0f/w;

        ekf->V[1][0] = -w*c->l/rml2*(sin(theta_)-sin(ekf->theta)) + rpl/(2*rml)*cos(theta_);
        ekf->V[1][1] = (-w*c->l)/rml2*(-cos(theta_)+cos(ekf->theta)) + rpl/(2*rml)*sin(theta_);
        ekf->V[1][2] = 1.0f/w;
    }
    else{
        ekf->V[0][0] = 0.5*(cos(ekf->theta) + c->l/w*sin(ekf->theta));
        ekf->V[0][1] = 0.5*(sin(ekf->theta) - c->l/w*cos(ekf->theta));
        ekf->V[0][2] = -1.0f/w;

        ekf->V[1][0] = 0.5*(-c->l/w*sin(ekf->theta) + cos(ekf->theta));
        ekf->V[1][1] = 0.5*(c->l/w*cos(ekf->theta) + sin(ekf->theta));
        ekf->V[1][2] = 1.0f/w;
    }
}
